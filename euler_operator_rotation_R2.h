...
namespace Atmospheric_Flow {
  ...
  public:
    ...
    void vmult_gravity(Vec& dst, const Vec& src) const; /*--- Action of matrix 'Mg'. ---*/
    void vmult_coriolis(Vec& dst, const Vec& src) const; /*--- Action of matrix 'R'. ---*/
    ...
    void vmult_inv_I_plus_betaJ(Vec &dst, const Vec &src) const;

    void assemble_cell_term_inv_I_plus_betaJ(const MatrixFree<dim, Number>      &data,
                                         Vec                                &dst,
                                         const Vec                          &src,
  ...
  private:
    ...
    /*-- Assembler function for the 'Mg' matrix. ---*/
    void assemble_cell_term_gravity (const MatrixFree<dim, Number>&       data,
                                     Vec&                                 dst,
                                     const Vec&                           src,
                                     const std::pair<unsigned, unsigned>& cell_range) const;
                                     
    /*-- Assembler function for the 'R' matrix. --*/
    void assemble_cell_term_coriolis (const MatrixFree<dim, Number>&       data,
                                      Vec&                                 dst,
                                      const Vec&                           src,
                                      const std::pair<unsigned, unsigned>& cell_range) const;
      ...
      for(unsigned cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            if constexpr(dim == 2) {
              Tensor<1, dim, VectorizedArray<Number>> u_s_tmp;
              u_s_tmp[0] = u_s[0];
              u_s_tmp[1] = u_s[2];
              flux += a[IMEX_stage - 1][s - 1]*dt*(rho_s*u_s_tmp);
            }
            else {
  ...
  assemble_rhs_cell_term_momentum(const MatrixFree<dim, Number>&       data,
    ...
    if(IMEX_stage <= n_stages) {
      ...
      for(unsigned cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_u_bar.gather_evaluate(u_bar, EvaluationFlags::values);
        ...
        for(unsigned q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            if constexpr(dim == 2) {
              ...
              flux += a[IMEX_stage - 1][s - 1]*dt*(rho_s*tensor_product_u_s_tmp);
              flux += a_tilde[IMEX_stage - 1][s - 1]*dt*(inv_Ma2*p_s_times_identity);
            }
            else {
            ...
            /*const auto& k_cross_u = cross_product_3d(e_k, u_s);
            rotation_term += a_tilde[IMEX_stage - 1][s - 1]*dt*(inv_Ro*rho_s*k_cross_u);*/
            rotation_term[0] += a_tilde[IMEX_stage - 1][s - 1]*dt*(-inv_Ro*rho_s*u_rel[1]);
            rotation_term[1] += a_tilde[IMEX_stage - 1][s - 1]*dt*(inv_Ro*rho_s*u_rel[0]);
          }

          /*--- Add last contribution of the gravity and rotation terms (implicit treatment) ---*/
          const auto& rho_s = phi_rho.back().get_value(q);
          gravity_term += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*(inv_Fr2*rho_s*e_k);

          const auto& u_fixed   = phi_u.back().get_value(q);
          const auto& u_bar = phi_u_bar.get_value(q);
          const auto& u_rel= u_fixed - u_bar;

          /*const auto& k_cross_u = cross_product_3d(e_k, u_fixed);
          rotation_term += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*(inv_Ro*rho_s*k_cross_u);*/
          //rotation_term[0] += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*(-inv_Ro*rho_s*u_rel[1]);
          //rotation_term[1] += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*(inv_Ro*rho_s*u_rel[0]);
          phi.submit_value(rho_old*u_old - gravity_term - rotation_term, q);
          phi.submit_gradient(flux, q);
        }
        ...
        for(unsigned q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            if constexpr(dim == 2) {
              ...
              flux += b[s - 1]*dt*(rho_s*tensor_product_u_s_tmp);
              flux += b_tilde[s - 1]*dt*(inv_Ma2*p_s_times_identity);
            }
            else {
            ...
            /*const auto& k_cross_u = cross_product_3d(e_k, u_s);
            rotation_term += b_tilde[s - 1]*dt*(inv_Ro*rho_s*k_cross_u);*/
            rotation_term[0] += b_tilde[s - 1]*dt*(-inv_Ro*rho_s*u_rel[1]);
            rotation_term[1] += b_tilde[s - 1]*dt*(inv_Ro*rho_s*u_rel[0]);
          }
      ...
      for(unsigned cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            if constexpr(dim == 2) {
              ...
              flux += a[IMEX_stage - 1][s - 1]*dt*
                      (rho_s*(0.5*Ma2*scalar_product(u_s, u_s))*u_s_tmp)
                    + a_tilde[IMEX_stage - 1][s - 1]*dt*
                      (inv_Gamma*(pres_s*u_s_tmp));
            }
            else {
              flux += a[IMEX_stage - 1][s - 1]*dt*
          ...
          //gravity_term += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*
                          //(Ma2_ov_Fr2*rho_for_fixed_s*u_fixed_s[2]);
  ...
  // Assemble cell term for the contribution due to gravity term 
  ...
  void EULEROperator<dim,
                     ...
                     n_q_points_1d, n_q_points_1d_boundary,
                     Vec>::
  assemble_cell_term_gravity(const MatrixFree<dim, Number>&               data,
                             Vec&                                         dst,
                             const Vec&                                   src,
                             const std::pair<unsigned, unsigned>& cell_range) const {
    FEEvaluation_u   phi_u(data, EquationData::U_INDEX_DOF);
    FEEvaluation_rho phi_rho(data, EquationData::RHO_INDEX_DOF);  
    FEEvaluation_pres phi_pres(data, EquationData::P_INDEX_DOF);

    for(unsigned cell = cell_range.first; cell < cell_range.second; ++cell) {
      phi_rho.reinit(cell);
      phi_rho.gather_evaluate(rho_for_fixed, EvaluationFlags::values);
      phi_u.reinit(cell);
      phi_u.gather_evaluate(src, EvaluationFlags::values);
      phi_pres.reinit(cell);
      

      for(unsigned q = 0; q < phi_rho.n_q_points; ++q) {
        const auto& rho = phi_rho.get_value(q);
        const auto& u   = phi_u.get_value(q);

        phi_pres.submit_value(+a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*
                              (Ma2_ov_Fr2*rho*u[2]), q);
      }

      phi_pres.integrate_scatter(EvaluationFlags::values, dst);
    }
  }

  /*-- Assemble cell term for the contribution due to Coriolis term --*/
  template<unsigned dim,
           unsigned fe_degree_u, unsigned fe_degree_rho, unsigned fe_degree_p,
           unsigned n_q_points_1d, unsigned n_q_points_1d_boundary,
           typename Vec>
  void EULEROperator<dim,
                     ...
                     n_q_points_1d, n_q_points_1d_boundary,
                     Vec>::
  assemble_cell_term_coriolis(const MatrixFree<dim, Number>&               data,
                              Vec&                                         dst,
                              const Vec&                                   src,
                              const std::pair<unsigned, unsigned>& cell_range) const {
    FEEvaluation_u phi_u(data, EquationData::U_INDEX_DOF);
    ...
    for(unsigned cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      for(unsigned q = 0; q < phi_rho.n_q_points; ++q) {
        ...
        Tensor<1, 3, VectorizedArray<Number>> coriolis_term;
        coriolis_term[0] += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*(-inv_Ro*rho*u[1]);
        coriolis_term[1] += a_tilde[IMEX_stage - 1][IMEX_stage - 1]*dt*(inv_Ro*rho*u[0]);
        
        phi_u.submit_value(coriolis_term, q);
      }

      phi_u.integrate_scatter(EvaluationFlags::values, dst);
    }
  }
  //////////////////////////////////////////////////////////////
  ...
  apply_add(Vec& dst, const Vec& src) const {
    ...
    if(Euler_stage == EquationData::RHO_INDEX_SYSTEM) {
      ...
      if(IMEX_stage <= n_stages) {
        ...
        this->vmult_pressure(tmp_1, src); // tmp_1 = B*P
        ...
        Vec y;
        this->data->initialize_dof_vector(y, EquationData::U_INDEX_DOF);
        this->vmult(y, tmp_1); //y =inv(A)*tmp_1

        Vec tmp_2;
        this->data->initialize_dof_vector(tmp_2, EquationData::U_INDEX_DOF);
        this->vmult_inv_I_plus_betaJ(tmp_2, y); // tmp_2 = inv(A+R)*(B*P)
        ...
        Vec tmp_4; 
        this->data->initialize_dof_vector(tmp_4, EquationData::P_INDEX_DOF);
        this->vmult_gravity(tmp_4, tmp_2);
        dst.add(static_cast<Number>(-1.0), tmp_4);
  ...
  // Application of the gravity matrix
  template<unsigned dim,
           unsigned fe_degree_u, unsigned fe_degree_rho, unsigned fe_degree_p,
           unsigned n_q_points_1d, unsigned n_q_points_1d_boundary,
           typename Vec>
  void EULEROperator<dim,
                     ...
                     n_q_points_1d, n_q_points_1d_boundary,
                     Vec>::
  vmult_gravity(Vec& dst, const Vec& src) const {
    src.update_ghost_values();
    rho_for_fixed.update_ghost_values();
    this->data->cell_loop(&EULEROperator::assemble_cell_term_gravity,
                     this, dst, src, true);

    }

  // Application of the Coriolis matrix
  template<unsigned dim,
           unsigned fe_degree_u, unsigned fe_degree_rho, unsigned fe_degree_p,
           unsigned n_q_points_1d, unsigned n_q_points_1d_boundary,
           typename Vec>
  void EULEROperator<dim,
                     ...
                     n_q_points_1d, n_q_points_1d_boundary,
                     Vec>::
  vmult_coriolis(Vec& dst, const Vec& src) const {
    src.update_ghost_values();
    rho_for_fixed.update_ghost_values();
    this->data->cell_loop(&EULEROperator::assemble_cell_term_coriolis,
                     this , dst, src, true); 
    }
  ...
  vmult_inv_I_plus_betaJ(Vec &dst, const Vec &src) const
    {
      src.update_ghost_values();
      this->data->cell_loop(&EULEROperator::assemble_cell_term_inv_I_plus_betaJ,
                            this, dst, src,
  ...
  void EULEROperator<dim,fe_degree_u,fe_degree_rho,fe_degree_p,
                      n_q_points_1d,n_q_points_1d_boundary,Vec>::
  assemble_cell_term_inv_I_plus_betaJ(const MatrixFree<dim, Number>       &data,
                                        Vec                                 &dst,
                                        const Vec                           &src,
                                        const std::pair<unsigned,unsigned>  &cell_range) const
    {
      ...
      const Number beta = a_tilde[IMEX_stage - 1][IMEX_stage - 1] * dt * inv_Ro;
      const Number s    = Number(1.0) / (Number(1.0) + beta*beta);
      ...
      for (unsigned cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        // Operazione dof-wise sulle 3 componenti
        for (unsigned i = 0; i < phi.dofs_per_component; ++i)
        {
          const auto v = phi.get_dof_value(i); // Tensor<1,3>

          Tensor<1,3,VectorizedArray<Number>> out;
          // (I + beta J)^(-1) con J(u,v,w)=(-v,u,0)
          out[0] = s * (v[0] + beta * v[1]);   // u
          out[1] = s * (v[1] - beta * v[0]);   // v
          out[2] = v[2];                       // w invariata

          phi.submit_dof_value(out, i);
        }
