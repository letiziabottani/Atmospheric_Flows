...
*/
  ...
  ICBC::Density_Bar<dim, Number> rho_bar_init;
  ...
  std::array<Number, 3> compute_max_Cu_per_direction() const; /*--- Get maximum Courant numbers along all the directions ---*/
...
EulerSolver<dim>::EulerSolver(const RunTimeParameters::Data_Storage& data,
  ...
  /*--- Domain ---*/
  /*
  push_forward(data.z_max, data.d, data.xc, data.L_ref),
  pull_back(data.z_max, data.d, data.xc, data.L_ref),
  manifold(push_forward, pull_back),
  /*--- Initial condition ---*/
  rho_init(data.rho_ref, data.L_ref, data.T_ref, data.H, data.xc, data.ac,
           data.initial_time),
  u_init(data.u_bar, data.u_ref, data.initial_time),
  pres_init(data.p_ref, data.L_ref, data.T_ref,
            data.initial_time),
  rho_bar_init(data.p_ref, data.L_ref,data.T_ref,
               data.initial_time),
  /*--- Output ---*/
  saving_dir(data.dir),
...
void EulerSolver<dim>::setup_dofs(const RunTimeParameters::Data_Storage& data) {
  ...
  /*--- Initialize the variables related to the temperature ---*/
  matrix_free_storage->initialize_dof_vector(T_old, EquationData::P_INDEX_DOF);
  matrix_free_storage->initialize_dof_vector(T_prime, EquationData::P_INDEX_DOF);
  matrix_free_storage->initialize_dof_vector(T_bar, EquationData::P_INDEX_DOF);
  ...
  for(const auto& cell: dof_handler_pressure.active_cell_iterators()) {
    if(cell->is_locally_owned()) {
      ...
      for(unsigned idx = 0; idx < dof_indices.size(); ++idx) {
        ...
        const auto T_b = data.T_bar;
        //const auto T_b  = p_bar/(static_cast<Number>(EquationData::R)*rho_bar(dof_indices[idx]));
        const auto Pi_bar = std::pow(p_bar/p_ref, Gamma);
        T_bar(dof_indices[idx]) = T_b;
        theta_bar(dof_indices[idx]) = T_b/Pi_bar;
      }
    }
  }
  ...
  for(const auto& cell: dof_handler_pressure.active_cell_iterators()) {
    if(cell->is_locally_owned()) {
      ...
      for(unsigned idx = 0; idx < dof_indices.size(); ++idx) {
        ...
        const auto Pi   = std::pow(pres/p_ref, Gamma);
        theta_old(dof_indices[idx]) = T/Pi;
        T_old(dof_indices[idx])= T;
      }
    }
  }
  T_old.update_ghost_values();
  data_out.add_data_vector(dof_handler_pressure, T_old, "T", {DataComponentInterpretation::component_is_scalar});

  T_prime.equ(static_cast<Number>(1.0), T_old);
  T_prime.add(static_cast<Number>(-1.0), T_bar);
  T_prime.update_ghost_values();
  data_out.add_data_vector(dof_handler_pressure, T_prime, "T_prime", {DataComponentInterpretation::component_is_scalar});
...
typename EulerSolver<dim>::Number EulerSolver<dim>::get_max_velocity() const {
  ...
  std::vector<Vector<Number>> velocity_values(n_q_points, Vector<Number>(3));
...
//
template<unsigned dim>
std::array<typename EulerSolver<dim>::Number, 3>
EulerSolver<dim>::compute_max_Cu_per_direction() const {
  ...
  std::vector<Vector<Number>> solution_values_velocity(n_q_points, Vector<Number>(3));

  std::array<Number, 3> res;
    ...
    if(cell->is_locally_owned()) {
      ...
      for(unsigned q = 0; q < n_q_points; ++q) {
        for(unsigned d = 0; d < 3; ++d) {
          if(d == 0) {
            res[0] = std::max(res[0],
                              EquationData::degree_u*(std::abs(solution_values_velocity[q](0))*dt/static_cast<Number>(cell->extent_in_direction(0))));
          }
          else if(d == 1) {
            res[1] = std::abs(solution_values_velocity[q](1));
          }
          else {
            res[2] = std::max(res[2],
                              EquationData::degree_u*(std::abs(solution_values_velocity[q](2))*dt/static_cast<Number>(cell->extent_in_direction(1))));
          }
        }
      }
    }
...
void EulerSolver<dim>::run(const bool verbose,
  ...
  while(std::abs(T - time) > static_cast<Number>(1e-10)) {
    ...
    const auto max_C_x_z = compute_max_C_per_direction();
    pcout << "CFL_c_x = " << max_C_x_z[0] << std::endl;
    pcout << "CFL_c_z = " << max_C_x_z[1] << std::endl;
